...
def home():   
    ...
    if CovidTest.query.all(): 
        ...
        df['confirmed_count'] = df['recovered_count'] = df['deceased_count'] = total_tests_done = positive_result_percentage = negative_result_percentage = df['negative_results'] =0          
    ...
    if Hospitals.query.all():
        ...
        vaccination_result = db.session.query(func.sum(Hospitals.first_dose),func.sum(Hospitals.second_dose), func.sum(Hospitals.precautionary_dose)).all()             
    ...
    if Patients.query.all():
        ...
        age_1,age_2,age_3,age_4, = age_result[0][0],age_result[0][1],age_result[0][2],age_result[0][3]
    else:
        male_percentage = female_percentage = age_1 = age_2 = age_3 = age_4 = 0
...
def my_scheduled_job():
    if db.session.query(func.count(Patients.status)).filter_by(status = 'positive').one():
